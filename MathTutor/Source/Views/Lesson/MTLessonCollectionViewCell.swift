...
class MTLessonCollectionViewCell: UICollectionViewCell {
    
    static let reuseIdentifier = "MTLessonCollectionViewCell"
    static let nibName = "MTLessonCollectionViewCell"

    let configController = ConfigController.sharedInstance
    let dataManager = BCGSDataManager.sharedInstance
    let userManager = MTUserManager.shared
    ...
    static func cellHeight() -> CGFloat {
        let height: CGFloat = UIDevice.isPad ? 150.0 : 110.0
        return height
    }
    
    static func cellWidth(for collectionView: UICollectionView, fullOnPhone: Bool = false) -> CGFloat {
        var minWidth = fullOnPhone ? floor(collectionView.bounds.width) : floor(collectionView.bounds.width * 0.8)
        
        if UIDevice.isPad {
            if let layout = collectionView.collectionViewLayout as? UICollectionViewFlowLayout {
                let insetWidth = collectionView.contentInset.left + collectionView.contentInset.right
                let width: CGFloat = collectionView.frame.size.width - insetWidth - layout.minimumInteritemSpacing * 2
                minWidth = floor(width / 2.1)
            }
        }
        
        let width = min(400, minWidth)
        return width
    }
    ...
    override func awakeFromNib() {
        ...
        let fontIncrement: CGFloat = UIDevice.isPad ? 2.0 : -2.0
                ...
                if let context = storage.mainContext {
                    if self.favoriteButton.imageView?.image != #imageLiteral(resourceName: "favorite_selected") {
                        if videos.count >= GlobalConstants.Maximum.favoriteCount {
                            ...
                            let count = GlobalConstants.Maximum.favoriteCount
                            let localizedVersion = String.localizedStringWithFormat(formatString, count)
